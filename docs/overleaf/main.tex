...
Les performances du $K$-means sont fortement conditionnées par trois facteurs structurels, analysés ici pour la meilleure variante (LAB).
...
\caption{Courbes d'entraînement du U-Net final. À gauche~: coefficient de Dice (train et validation) en fonction de l'epoch. À droite~: loss (Dice loss). La ligne horizontale pointillée indique le meilleur Dice de validation ($0{,}894$, epoch~20).}
...
\begin{enumerate}
    ...
    \item \textbf{Analyse d'erreurs par sous-groupes.} Une évaluation stratifiée par taille de lésion, niveau de contraste et présence d'artefacts permettrait d'identifier les cas résiduels de défaillance et de guider les améliorations futures.
